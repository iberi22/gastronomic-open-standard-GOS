...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (
      estimatedTokens > 0 &&
      estimatedTokens > ledger.remaining &&
      tierId !== 'socio-managed'
    ) {
      return json(
        {
          error: '402 Payment Required - credito agotado, upgrade socio',
          ledger,
        },
        402,
        { 'X-GOS-Paywall': '402 - D1/KV credit exhausted' },
      )
    }
    ...
    return json(
      {
        ...
        message:
          'JWT emitido via D1/KV billing 20% handling - usar Authorization: Bearer <jwt> para /graph-data.json escalado',
      },
      200,
      { 'X-GOS-Paywall': 'D1/KV - JWT issued' },
    )
  } catch (e) {
...
export const GET: APIRoute = async () => {
  return json(
    {
      error:
        "use POST { tier: 'socio', turnstile, estimatedTokens } - see site/src/lib/billing.ts",
    },
    405,
  )
}
