...
class MemoryAdapter implements StorageAdapter {
  ...
  async list(entity: string, instanceId: string): Promise<DomainRecord[]> {
    return [...this.key(entity).values()].filter(
...
async function autoDetectAdapter(): Promise<StorageAdapter> {
  ...
  try {
    const { IndexedDBStorageAdapter, isIndexedDBAvailable } = await import(
      './indexeddb'
    )
...
function assertEntity(entity: string) {
  ...
  if (!ok)
    throw new Error(
      `Entity no whitelisteada en domain.config: ${entity}. Define entities[] primero.`,
    )
}
...
export function surrealDefineTable(entity: string): string {
  ...
  if (!def) throw new Error(`Entidad no whitelisteada: ${entity}`)
  const fields = def.fields
    .map((f) => `DEFINE FIELD ${f} ON ${entity} TYPE string;`)
    .join('\n')
  return [
...
export async function createEntity<T extends Record<string, unknown>>(
  ...
  return (await adapter.create(
    entity as string,
    record as unknown as DomainRecord,
  )) as DomainRecord<T>
}
