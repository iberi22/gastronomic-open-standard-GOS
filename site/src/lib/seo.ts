...
export function recipeJsonLd(input: RecipeLDInput) {
  return {
    ...
    author: input.author
      ...
      : { '@type': 'Organization', name: 'GOS' },
    datePublished: input.datePublished,
    description:
      input.description ??
      `Receta ${input.title} del Gastronomic Open Standard`,
    ...
    isPartOf: {
      '@type': 'WebSite',
      name: 'Gastronomic Open Standard',
      url: SITE_URL,
    },
  }
}
...
export function ingredientJsonLd(input: IngredientLDInput) {
  ...
  return {
    ...
    description:
      input.description ??
      `${input.name} — ${input.group || 'ingrediente'} del grafo GOS`,
    ...
    isPartOf: {
      '@type': 'Dataset',
      name: 'GOS Ingredient Encyclopedia',
      url: absUrl('/ingredients'),
    },
  }
}
...
export function substanceJsonLd(input: SubstanceLDInput) {
  const d = input.data
  const studies = (d.health_registry ?? [])
    .flatMap((h) => h.studies ?? [])
    .map((s) => ({
  ...
  return {
    ...
    description:
      d.benefit ?? d.sazon ?? `Sustancia bioactiva ${d.name} del GOS`,
    additionalProperty: [
      d.discovery_year
        ? {
            '@type': 'PropertyValue',
            name: 'discovery_year',
            value: d.discovery_year,
          }
        : null,
      d.source_ingredient
        ? {
            ...
            value: d.source_ingredient,
          }
        : null,
      d.sazon
        ? { '@type': 'PropertyValue', name: 'sazon', value: d.sazon }
        : null,
      d.sabor
        ? { '@type': 'PropertyValue', name: 'sabor', value: d.sabor }
        : null,
      d.textura
        ? { '@type': 'PropertyValue', name: 'textura', value: d.textura }
        : null,
    ].filter(Boolean),
    contains: d.vitaminas?.map((v) => ({
      '@type': 'Vitamin' as unknown as string,
      name: v,
    })),
    isRelatedTo: d.compuestos?.map((c) => ({
      '@type': 'ChemicalSubstance',
      name: c,
    })),
