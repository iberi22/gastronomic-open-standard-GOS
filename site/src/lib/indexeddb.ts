...
export class IndexedDBStorageAdapter implements StorageAdapter {
  async create(entity: string, record: DomainRecord): Promise<DomainRecord> {
    ...
    await promisifyRequest(store.put({ ...record, updated_at: new Date().toISOString() }));
    return record;
  }
  ...
  async list(entity: string, instanceId: string): Promise<DomainRecord[]> {
    ...
    const store = tx.objectStore(entity as EntityStore);
    const all = await promisifyRequest<DomainRecord[]>(store.getAll() as IDBRequest<DomainRecord[]>);
  ...
  async get(entity: string, id: string, instanceId: string): Promise<DomainRecord | null> {
    ...
    const tx = db.transaction(entity as EntityStore, 'readonly');
    const store = tx.objectStore(entity as EntityStore);
    const record = await promisifyRequest<DomainRecord | undefined>(store.get(id));
