...
export class D1StorageAdapter implements StorageAdapter {
  ...
  async create(entity: string, record: DomainRecord): Promise<DomainRecord> {
    ...
    const values = cols.map((k) => serializeForD1(record[k as keyof DomainRecord]));
    await this.db
  ...
  async list(entity: string, instanceId: string): Promise<DomainRecord[]> {
    const { results } = await this.db
      .prepare(`SELECT * FROM ${entity} WHERE instance_id = ? ORDER BY updated_at DESC`)
      .bind(instanceId)
      .all<DomainRecord>();
    return results || [];
  }
  ...
  async update(
    ...
    const values = cols.map((k) => serializeForD1((next as Record<string, unknown>)[k]));
    await this.db
      .prepare(`UPDATE ${entity} SET ${setClause} WHERE id = ? AND instance_id = ?`)
