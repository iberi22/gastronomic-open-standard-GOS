...
export const AI_MARGIN_MIN_PCT = 0.1 // 10% minimo sobre costo base Workers AI (Cloudflare)
...
export function creditStatus(
  used: number,
  tierId: SocioTier['id'] = 'socio',
): CreditLedger {
  const limit = TIERS[tierId]?.monthlyCredit ?? 0
  return { used, limit, remaining: Math.max(0, limit - used) }
}

export function canAffordInference(
