...
import { type XavierMemory, xavierAdd, xavierSearch } from './xavier'
...
export async function agentCreate<T extends Record<string, unknown>>(
  entity: EntityName,
  data: T,
  instanceId?: string,
  ...
  try {
    await xavierAdd(
      toMemoryContent(entity as string, rec, 'create'),
      xavierKindFor(entity as string),
      iid,
    )
  } catch {}
  try {
    await meshPublish(`${entity}:created`, rec, iid)
  } catch {}
  // tambien AUI update via mesh
  try {
    await meshPublish(
      'aui:update',
      { entity, id: rec.id, action: 'create' },
      iid,
    )
  } catch {}
  return rec
...
export async function agentList<T = Record<string, unknown>>(
  entity: EntityName,
  instanceId?: string,
): Promise<DomainRecord<T>[]> {
...
export async function agentGet<T = Record<string, unknown>>(
  entity: EntityName,
  id: string,
  instanceId?: string,
...
export async function agentUpdate<T extends Record<string, unknown>>(
  entity: EntityName,
  id: string,
  patch: Partial<T>,
  instanceId?: string,
  ...
  try {
    await xavierAdd(
      toMemoryContent(entity as string, rec, 'update'),
      xavierKindFor(entity as string),
      iid,
    )
  } catch {}
  try {
    await meshPublish(`${entity}:updated`, rec, iid)
  } catch {}
  return rec
}
...
export async function agentDelete(
  ...
  if (ok && cur) {
    try {
      await xavierAdd(
        toMemoryContent(entity as string, cur, 'delete'),
        xavierKindFor(entity as string),
        iid,
      )
    } catch {}
    try {
      await meshPublish(`${entity}:deleted`, { id, instance_id: iid }, iid)
    } catch {}
  }
  return ok
}
...
export async function agentSearch<T = Record<string, unknown>>(
  entity: EntityName,
  query: string,
  instanceId?: string,
  limit = 5,
): Promise<{ memories: XavierMemory[]; records: DomainRecord<T>[] }> {
  const iid = (instanceId ?? (domainConfig.instanceId as string)) as string
