...
export const AUI_LOCKED_KEYS = [
  'manager_adds_vote_weight',
  'synapse_unfrozen',
  'token_public_sale_enabled',
] as const
...
export async function generateAui(
  prompt: string,
  useMemory = true,
): Promise<AuiSpec> {
...
export function validateAui(spec: unknown): AuiSpec {
  const s = spec as Record<string, unknown>
  if (s?.version !== 1 || !Array.isArray(s?.components)) {
    throw new Error('AUI invalid version/components')
  }
  // whitelist + lockedKeys
  if (
    (s.lockedKeys as string[] | undefined)?.some((k: string) =>
      (AUI_LOCKED_KEYS as readonly string[]).includes(k),
    )
  ) {
  ...
  for (const c of s.components) {
    ...
    if (!(AUI_COMPONENTS as readonly string[]).includes(comp.type as string))
      throw new Error(`AUI component not whitelisted: ${comp.type}`)
    if (!comp.id || typeof comp.id !== 'string')
      throw new Error('AUI missing id')
    if (typeof comp.props !== 'object')
      throw new Error('AUI props must be object')
  }
