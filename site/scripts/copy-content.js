...
const placeholderPath = path.join(
  repoRoot,
  'dishes/colombian/nacionales/img/chorizo-2314640_640.jpg',
)
const baseUrl =
...
async function processImage(srcPath, destPath, isLfs) {
  try {
    ...
    const resolvedIsLfs = prefix.startsWith(
    ...
    if (OPTIMIZE_EXTENSIONS.includes(ext) && !resolvedIsLfs) {
      ...
      try {
        ...
        console.warn(
          `Sharp failed for ${srcPath}, falling back to copy. Error: ${sharpError.message}`,
        )
          ...
          content = content.replace(
            ...
            (match, imgPath) => {
              try {
                ...
                const publicUrl =
                  `${baseUrl}/${urlSegment}/${newRelativePath.replace(entry.name, '')}${cleanPath}`.replace(
                    /\/+/g,
                    '/',
                  )
...
;(async () => {
  try {
    ...
    console.log(
      'Copying dishes to site content and public (with optimizations)...',
    )
    if (fs.existsSync(dishesDir)) {
      const entries = await fsPromises.readdir(dishesDir, {
        withFileTypes: true,
      })
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          await copyDir(
            srcPath,
            path.join(dishesContentDir, entry.name),
            path.join(dishesPublicDir, entry.name),
            entry.name,
            'dishes',
          )
    ...
    if (fs.existsSync(tipsDir)) {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          await copyDir(
            srcPath,
            path.join(tipsContentDir, entry.name),
            path.join(tipsPublicDir, entry.name),
            entry.name,
            'tips',
          )
        } else if (entry.isFile() && entry.name.endsWith('.md')) {
