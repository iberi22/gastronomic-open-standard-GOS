...
function normalizeDifficulty(diff) {
  ...
  if (
    dLower.includes('easy') ||
    dLower.includes('baja') ||
    dLower.includes('fácil') ||
    dLower.includes('facil')
  ) {
...
function determineLanguage(filePath, existingLang) {
  if (
    existingLang &&
    ['es', 'zh', 'pt', 'en'].includes(String(existingLang).toLowerCase())
  ) {
