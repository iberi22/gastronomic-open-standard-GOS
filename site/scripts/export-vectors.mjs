  ...
  for (const file of files) {
    try {
      ...
      const healthConds = Array.isArray(data.health_registry)
        ? data.health_registry
            .map((h) => h.condition || h.name || '')
            .filter(Boolean)
            .join(', ')
        : ''
      ...
      console.warn(
        `[export-vectors] Error parsing ingredient ${file}:`,
        err.message,
      )
    }
  }
  ...
  for (const file of files) {
    try {
      ...
      const categories = Array.isArray(data.categories)
        ...
        : data.category || ''
      const mainIngs = Array.isArray(data.main_ingredients)
        ? data.main_ingredients.join(', ')
        : ''
...
function scanSubstancesCollection() {
  ...
  for (const file of files) {
    try {
      ...
      console.warn(
        `[export-vectors] Error parsing substance ${file}:`,
        err.message,
      )
    }
  }
...
class WorkersAIProvider {
  ...
  async init() {
    if (!this.accountId || !this.apiToken) {
      throw new Error(
        'Workers AI requires CF_ACCOUNT_ID and CLOUDFLARE_API_TOKEN',
      )
    }
    console.log(
      `[export-vectors] Configured Cloudflare Workers AI provider: ${this.name}`,
    )
  }
...
async function getProvider() {
  ...
  if (providerType === 'workers-ai' || process.argv.includes('--workers-ai')) {
    try {
      ...
      console.warn(
        `[export-vectors] Workers AI initialization failed (${err.message}). Falling back to local transformers provider.`,
      )
    }
  }
...
export async function exportVectors() {
  ...
  for (let i = 0; i < allItems.length; i += batchSize) {
    ...
    if ((i + batchSize) % 128 === 0 || i + batchSize >= allItems.length) {
      console.log(
        `   Processed ${Math.min(i + batchSize, allItems.length)} / ${totalCount} items`,
      )
    }
  }

  // Spot-check Cosine Similarity for 'ajo' vs 'garlic'
  const ajoItem = vectors.find(
    (v) =>
      v.type === 'ingredient' &&
      (v.id.endsWith('/ajo') ||
        v.id === 'ajo' ||
        v.id.includes('condiments/ajo')),
  )
  if (ajoItem) {
    const [garlicEmbedding] = await provider.embedBatch([
      'Garlic (ajo) - Allium sativum. Group: Condiment. Bioactivo: alicina',
    ])
    const sim = cosineSimilarity(ajoItem.embedding, garlicEmbedding)
    console.log(
      `🎯 Spot-check Cosine Similarity [ajo vs garlic]: ${sim.toFixed(4)}`,
    )
    if (sim < 0.8) {
      throw new Error(
        `Spot-check failed: cosine similarity ${sim.toFixed(4)} < 0.8`,
      )
    } else {
  ...
  for (let i = 0; i < vectors.length; i += maxItemsPerShard) {
    ...
    console.log(
      `   ✓ Written ${shardFileName} (${shardItems.length} vectors, ${(stats.size / 1024 / 1024).toFixed(2)} MB)`,
    )
  }
