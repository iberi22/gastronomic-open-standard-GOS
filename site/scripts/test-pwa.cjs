...
function log(label, status, msg) {
  const color = status === FAIL ? '\x1b[31m' : status === WARN ? '\x1b[33m' : '\x1b[32m';
  const reset = '\x1b[0m';
...
function checkServiceWorker() {
  ...
  const checks = [
    { key: 'install', test: c => c.includes("addEventListener('install'") || c.includes('addEventListener("install"'), label: 'install event' },
    { key: 'activate', test: c => c.includes("addEventListener('activate'") || c.includes('addEventListener("activate"'), label: 'activate event' },
    { key: 'fetch', test: c => c.includes("addEventListener('fetch'") || c.includes('addEventListener("fetch"'), label: 'fetch event' },
    { key: 'cacheFirst', test: c => c.includes('cacheFirst') || c.includes('cache first'), label: 'cache-first strategy' },
    { key: 'networkFirst', test: c => c.includes('networkFirst') || c.includes('network first'), label: 'network-first strategy' },
    { key: 'staleWhileRevalidate', test: c => c.includes('staleWhileRevalidate') || c.includes('stale while'), label: 'stale-while-revalidate strategy' },
    { key: 'CACHE_NAME', test: c => c.includes('CACHE_NAME'), label: 'cache name constant' },
    { key: 'skipWaiting', test: c => c.includes('skipWaiting'), label: 'skipWaiting call' },
  ];
...
function checkManifest() {
  ...
  required.forEach(field => {
    ...
    if (val !== undefined && val !== null && val !== '') {
      log(field, PASS, val === '' ? 'empty string' : String(val).substring(0, 50));
    } else {
...
function checkCachingStrategies() {
  ...
  const resourceChecks = [
    { pattern: /request\.destination\s*===\s*['"]style['"]|request\.destination\s*===\s*['"]script['"]|request\.destination\s*===\s*['"]image['"]|request\.destination\s*===\s*['"]font['"]/, label: 'Static assets use Cache First', test: 'static-cache' },
    { pattern: /pathname\.startsWith\s*\(\s*['"]\/api\/['"]\s*\)|hostname\.includes\s*\(\s*['"]github\.com['"]\s*\)/, label: 'API/GitHub use Network First', test: 'api-network' },
    { pattern: /request\.destination\s*===\s*['"]document['"]|HTML pages/i, label: 'HTML uses Stale While Revalidate', test: 'html-swr' },
  ];

  resourceChecks.forEach(({ pattern, label, test }) => {
    const found = pattern.test(code);
    log(label, found ? PASS : WARN, found ? 'Strategy found' : 'Strategy not clearly identified');
  });
}
