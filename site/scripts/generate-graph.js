...
const DIETS = [
  ...
  { id: 'diet_sin_gluten', label: 'Sin gluten', rule: 'Excluye trigo/cebada/centeno' },
        ...
        if (e.isDirectory()) walk(p);
          ...
          try {
            ...
            if (fm.micronutrients && typeof fm.micronutrients === 'object') {
              for (const [nutKey, val] of Object.entries(fm.micronutrients)) {
                const vitLabel = nutKey.replace(/_mg$|_ug$|_iu$/,'').replace(/_/g,' ').replace(/\b\w/g,c=>c.toUpperCase());
            ...
            if (fm.nutrition_per_100g) {
              ...
              for (const k of ['protein_g','fat_g','carbs_g','fiber_g']) {
                if (fm.nutrition_per_100g[k] != null) {
                  const nLabel = k.replace('_g','').replace(/\b\w/g,c=>c.toUpperCase());
                  const nId = `nutrient_${sanitizeId(nLabel)}`;
                  if (!nodes.has(nId)) nodes.set(nId, { id: nId, label: nLabel, type: 'nutrient', color: NODE_COLORS.nutrient, size: 12 });
                  edges.push({ source: ingId, target: nId, type: 'HAS_NUTRIENT', weight: 0.6 });
                }
              }
            }
            ...
            if (Array.isArray(fm.active_compounds)) {
              for (const c of fm.active_compounds) {
                ...
                if (!nodes.has(sId)) nodes.set(sId, { id: sId, label: subName, type: 'substance', color: NODE_COLORS.substance, size: 16, benefit: c.benefit || '' });
                edges.push({ source: ingId, target: sId, type: 'HAS_SUBSTANCE', weight: 1 });
                edges.push({ source: sId, target: ingId, type: 'FOUND_IN', weight: 0.7 });
              }
            }
            ...
            if (Array.isArray(fm.health_registry)) {
              for (const h of fm.health_registry) {
                ...
                if (!nodes.has(cId)) nodes.set(cId, { id: cId, label: condName, type: 'condition', color: NODE_COLORS.condition, size: 18, category: h.evidence_level || 'medium', mechanism: h.mechanism || '' });
                edges.push({ source: ingId, target: cId, type: 'HELPS_CONDITION', weight:  1.2 });
                if (Array.isArray(h.compounds)) {
                  for (const comp of h.compounds) {
                    const subId = `substance_${sanitizeId(comp)}`;
                    if (nodes.has(subId)) edges.push({ source: subId, target: cId, type: 'TREATS', weight: 1 });
                  }
                }
              }
            }
            ...
            if (Array.isArray(fm.tags)) {
              const tagMap = { 'sin gluten': 'diet_sin_gluten', 'vegano': 'diet_vegano', 'vegetariano': 'diet_vegetariano', 'keto': 'diet_keto', 'mediterranea': 'diet_mediterranea' };
              for (const tg of fm.tags) {
                const key = String(tg).toLowerCase();
                for (const [k, dietId] of Object.entries(tagMap)) {
                  if (key.includes(k)) {
                    const dietLabel = DIETS.find(d=>d.id===dietId)?.label || k;
                    if (!nodes.has(dietId)) nodes.set(dietId, { id: dietId, label: dietLabel, type: 'diet', color: NODE_COLORS.diet, size: 20 });
                    edges.push({ source: ingId, target: dietId, type: 'FITS_DIET', weight: 0.8 });
                  }
                }
              }
            }
