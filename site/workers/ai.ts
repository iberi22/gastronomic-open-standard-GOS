...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (
      ...
      return Response.json(
        { error: 'self-managed requiere X-CF-Account-Id' },
        { status: 400 },
      )
    }
    ...
    if (used >= tier.monthlyCredit) {
      return Response.json(
        { error: 'credito agotado', used, limit: tier.monthlyCredit },
        { status: 402 },
      )
    }
    ...
    if (used + estimated > tier.monthlyCredit) {
      return Response.json(
        {
          error: 'credito insuficiente para prompt',
          used,
          limit: tier.monthlyCredit,
          estimated,
        },
        { status: 402 },
      )
    ...
    try {
      await env.SWAL_D1.prepare(
        'INSERT OR REPLACE INTO credits (appId, used, tier, updatedAt) VALUES (?, ?, ?, ?)',
      )
    ...
    const { total, handling, aiWithMargin } = calculatePrice(
      infraCost,
      aiCostBase,
    )
    ...
    try {
      ...
      await env.SWAL_D1.prepare(
        'INSERT INTO invoices (id, appId, infra, aiBase, aiWithMargin, handling, total, tokensUsed) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      )
        .bind(
          id,
          appId,
    ...
    return Response.json({
      ...
      credit: {
        used: newUsed,
        limit: tier.monthlyCredit,
        remaining: tier.monthlyCredit - newUsed,
      },
    })
  },
}
