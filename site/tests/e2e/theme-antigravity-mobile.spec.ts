...
test.describe('Antigravity mobile - single tone clean fast', () => {
  for (const { name, device } of mobileViewports) {
    test(`mobile ${name} renders single tone without heavy borders`, async ({
      browser,
    }) => {
      ...
      const bg = await page.evaluate(() => {
        ...
        const token = getComputedStyle(html)
          .getPropertyValue('--swal-bg')
          .trim()
      ...
      const bgOk =
        ...
        (typeof bg.dataTheme === 'string' &&
          bg.dataTheme.startsWith('antigravity'))
      ...
      const swalFont = await page.evaluate(() =>
        getComputedStyle(document.documentElement)
          .getPropertyValue('--swal-font')
          .trim()
          .toLowerCase(),
      )
  ...
  test('grafos interconectados validan edges entre todos los tipos', async () => {
    ...
    expect(
      edgeTypes.has('USES') || edgeTypes.has('USES_INGREDIENT'),
    ).toBeTruthy()
    expect(
      edgeTypes.has('CONTAINS_VITAMIN') || edgeTypes.has('HAS_VITAMIN'),
    ).toBeTruthy()
    expect(edgeTypes.has('HAS_SUBSTANCE')).toBeTruthy()
    expect(
      edgeTypes.has('HELPS_CONDITION') || edgeTypes.has('TREATS'),
    ).toBeTruthy()
