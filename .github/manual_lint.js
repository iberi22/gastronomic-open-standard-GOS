...
// 校验函数集合
const validators = [
  async (filePath, _lines, errors) => {
    const filenameWithoutExt = path.parse(filePath).name // .name 是不带扩展名的文件名
    if (filenameWithoutExt.includes(' ')) {
      errors.push(
        `文件 ${filePath} 不符合仓库的规范！文件名不能包含空格！ (当前文件名: ${filenameWithoutExt})`,
      )
    }
  },
  ...
  async (filePath, lines, errors) => {
    ...
    if (!titles.length || titles[0] !== expectedMainTitle) {
      errors.push(
        `文件 ${filePath} 不符合仓库的规范！它的大标题应该是: "${expectedMainTitle}"! 而它现在是 "${titles[0] || '未找到主标题'}"!`,
      )
      return
    }
    ...
    if (sections.length !== requiredSections.length) {
      errors.push(
        `文件 ${filePath} 不符合仓库的规范！它并不是四个二级标题的格式 (应为 ${requiredSections.length} 个，实际 ${sections.length} 个)。请从示例菜模板中创建菜谱！请不要破坏模板的格式！`,
      )
      return
    }
    ...
    requiredSections.forEach((sec, idx) => {
      if (sections[idx] !== sec) {
        ...
        errors.push(
          `文件 ${filePath} 不符合仓库的规范！${titleName}标题不是 ${sec}! (当前为: "${sections[idx] || '未找到'}")`,
        )
      }
    })
    ...
    const firstSecondTitleIndex =
      sections.length > 0 ? lines.indexOf(sections[0]) : -1

    if (
      mainTitleIndex >= 0 &&
      firstSecondTitleIndex >= 0 &&
      mainTitleIndex < firstSecondTitleIndex
    ) {
      const contentBetweenTitles = lines.slice(
        mainTitleIndex + 1,
        firstSecondTitleIndex,
      )
      ...
      for (const line of contentBetweenTitles) {
        if (difficultyPatternGeneral.test(line)) {
          ...
          if (!difficultyPatternStrict.test(line)) {
            ...
            errors.push(
              `文件 ${filePath} 不符合仓库的规范！烹饪难度的星星数量必须在1-5颗之间！(当前为 ${starCount} 颗)`,
            )
          }
          break
        }
      ...
      if (!hasDifficultyLine) {
        errors.push(
          `文件 ${filePath} 不符合仓库的规范！在大标题和第一个二级标题之间必须包含"预估烹饪难度：★★"格式的难度评级，星星数量必须在1-5颗之间！`,
        )
      }
  ...
  async (filePath, lines, errors) => {
    ...
    if (
      count('勺') >
      count('勺子') + count('炒勺') + count('漏勺') + count('吧勺')
    ) {
    ...
    ;['适量', '少许'].forEach((w) => {
      if (count(w) > 0) {
        errors.push(
          `文件 ${filePath} 不符合仓库的规范！${w} 不是一个精准的描述！请给出克 g 或毫升 ml。`,
        )
      }
    })
    if (count('min') > 0) {
      errors.push(
        `文件 ${filePath} 不符合仓库的规范！min 这个词汇有多重含义。建议改成中文"分钟"。`,
      )
    }
    if (count('左右') > 0) {
      errors.push(
        `文件 ${filePath} 不符合仓库的规范！左右 不是一个能够明确定量的标准! 如果是在描述一个模糊物体的特征，请使用 '大约'。例如：鸡（大约1kg）`,
      )
    }
    ;['你', '我'].forEach((pronoun) => {
      if (count(pronoun) > 0) {
  ...
  async (filePath, lines, errors) => {
    ...
    const hasTemplateLine = lines.some((l) =>
      l.includes('每次制作前需要确定计划做几份。一份正好够'),
    )

    if (hasPortion && (!hasTotal || !hasTemplateLine)) {
      errors.push(
        `文件 ${filePath} 不符合仓库的规范！它使用份数作为基础，这种情况下一般是一次制作，制作多份的情况。请标明：总量 并写明 '每次制作前需要确定计划做几份。一份正好够 几 个人食用。'。`,
      )
    }
    if (lines.some((l) => l.includes('每人') || l.includes('人数'))) {
      errors.push(
        `文件 ${filePath} 不符合仓库的规范！请基于每道菜\\每份为基准。不要基于人数。人数是一个可能会导致在应用中发生问题的单位。如果需要面向大量的人食用，请标明一个人需要几份。`,
      )
    }
  },
  ...
  async (filePath, lines, errors) => {
    ...
    if (!lines.includes(footer)) {
      errors.push(
        `文件 ${filePath} 不符合仓库的规范！ 它没有包含必需的附加内容！，需要在最后一行添加模板中的【${footer}】`,
      )
    }
  },
]
...
async function main() {
  ...
  for (const p of allPaths) {
    ...
    if (stats.size > MAX_FILE_SIZE) {
      errors.push(
        `文件 ${p} 超过了1MB大小限制 (${(stats.size / 1048576).toFixed(2)}MB)! 请压缩图片或分割文件。`,
      )
    }
