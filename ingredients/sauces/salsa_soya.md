---
name: Salsa de Soya (Sillao)
scientific_name: Glycine max (fermented)
group: Sauce
image: ../../images/salsa_soya.jpg
portions:
  default_g: 15
nutrition_per_100g:
  calories: 53
  protein_g: 8
  fat_g: 0.6
  carbs_g: 4.9
  fiber_g: 0.8
micronutrients:
  sodium_mg: 5493
  manganese_mg: 1
active_compounds:
  - name: Isoflavones
    benefit: Phytoestrogens with antioxidant properties (varies by fermentation).
    scientific_ref: PubMed
health_registry:
  - condition: Digestive Secretion / Umami Stimulation
    mechanism: >-
      Free glutamate triggers T1R1/T1R3 umami taste receptors, stimulating
      gastric secretion and intestinal motility.
    compounds:
      - Glutamate
      - Isoflavones
    evidence_level: High
    studies:
      - title: Umami taste perception and salivary/gastric secretion
        source: American Journal of Clinical Nutrition
        year: 2009
        doi: 10.3945/ajcn.2009.27462J
        url: 'https://pubmed.ncbi.nlm.nih.gov/19640957/'
  - condition: Gastrointestinal Microbiota
    mechanism: >-
      Melanoidins produced during soy sauce fermentation act as prebiotics to
      improve gut microbial diversity.
    compounds:
      - Melanoidins
    evidence_level: Medium
    studies:
      - title: Prebiotic potential of fermented soy sauce melanoidins
        source: Journal of Agricultural and Food Chemistry
        year: 2014
        doi: 10.1021/jf501239e
        url: 'https://pubmed.ncbi.nlm.nih.gov/24854580/'
sources:
  - USDA FoodData Central
tags:
  - china
  - adobo
  - umami
aliases:
  es:
    - salsa de soya
    - salsa de soya clara
    - salsa de soya oscura
    - sillao
    - shoyu
  en:
    - soy sauce
    - dark soy sauce
    - light soy sauce
---

# Scientific Analysis & Culinary Use

## Description
La salsa de soya es el pilar del sabor en la cocina asiática. Es rica en glutamato natural (Umami).

## Culinary Uses
- **Arroz Chino:** Sabor y color oscuro.
- **Marinados:** Ablanda carnes por su acidez y enzimas.

## Health Benefits
### Umami y Apetito
El sabor Umami estimula la salivación y el apetito, útil en dietas para adultos mayores con anorexia, aunque debe controlarse por el sodio.
