---
name: "Cebolla Cabezona Blanca"
scientific_name: "Allium cepa"
group: "Vegetable"
portions:
  default_g: 100
nutrition_per_100g:
  calories: 40
  protein_g: 1.1
  fat_g: 0.1
  carbs_g: 9.3
  fiber_g: 1.7
  sugar_g: 4.2
micronutrients:
  vitamin_c_mg: 7.4
  folate_ug: 19
  potassium_mg: 146
active_compounds:
  - name: "Quercetin"
    benefit: "Potent antioxidant flavanol."
    scientific_ref: "NIH"
health_registry:
  - condition: "Inflammation"
    mechanism: "Quercetin inhibits production of COX-2 and LOX enzymes involved in inflammation."
    compounds: ["Quercetin", "Sulfur compounds"]
    evidence_level: "High"
    studies:
      - title: "Quercetin, Inflammation and Immunity"
        source: "Nutrients"
        year: 2016
        doi: "10.3390/nu8030167"
        url: "https://pubmed.ncbi.nlm.nih.gov/26978396/"
  - condition: "Diabetes Management"
    mechanism: "Hypoglycemic effects via increased insulin secretion and sensitivity."
    compounds: ["Sulfur compounds", "Quercetin"]
    evidence_level: "Medium"
    studies:
      - title: "Effect of raw and cooked onion on blood glucose levels in type 1 and type 2 diabetic patients"
        source: "Environmental Health Insights"
        year: 2010
        doi: "10.4137/EHI.S5540"
        url: "https://pubmed.ncbi.nlm.nih.gov/21151698/"
sources:
  - "USDA FoodData Central"
tags:
  - base
  - guiso
  - antiinflamatorio
---

# Scientific Analysis & Culinary Use

## Description
La cebolla blanca es la base aromática por excelencia. Rica en flavonoides, especialmente **quercetina**, que se concentra en las capas externas.

## Culinary Uses
- **Guiso/Hogao:** Sofrita con tomate.
- **Cruda:** En ensaladas o ceviches (aporta máxima actividad enzimática).
- **Caramelizada:** Libera azúcares naturales pero degrada algunos compuestos azufrados.

## Health Benefits
### Antiinflamatorio
La quercetina es uno de los antiinflamatorios naturales más estudiados, útil para condiciones crónicas.

### Salud Metabólica
El consumo regular se asocia con mejor control glucémico.
