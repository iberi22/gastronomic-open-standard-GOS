---
active_compounds:
  - benefit: Potent anti-inflammatory and anti-nausea compound.
    name: Gingerol
    scientific_ref: PubMed
aliases:
  es:
    - jengibre
    - jengibre fresco
    - jengibre rallado
    - kion
  en:
    - ginger
    - fresh ginger
    - root ginger
group: Condiment
health_registry:
  - compounds:
      - 6-Gingerol
      - Shogaol
    condition: Nausea / Motion Sickness
    evidence_level: High
    mechanism: >-
      Gingerols antagonize 5-HT3 receptors in the gut, reducing nausea without
      CNS side effects.
    studies:
      - doi: 10.4137/IMI.S36273
        source: Integrative Medicine Insights
        title: >-
          The Efficacy of Ginger in Preventing Nausea and Vomiting During
          Pregnancy and Chemotherapy
        url: 'https://pubmed.ncbi.nlm.nih.gov/27053918/'
        year: 2016
  - compounds:
      - Gingerol
      - Shogaol
    condition: Inflammation
    evidence_level: High
    mechanism: Inhibits COX-2 enzyme and lipoxygenase pathways similarly to NSAIDs.
    studies:
      - doi: 10.1002/art.20120
        source: Arthritis & Rheumatology
        title: >-
          Ginger (Zingiber officinale) in Rheumatism and Musculoskeletal
          Disorders
        url: 'https://pubmed.ncbi.nlm.nih.gov/11710709/'
        year: 2005
image: ../../images/jengibre.jpg
micronutrients:
  potassium_mg: 415
name: Jengibre
nutrition_per_100g:
  calories: 80
  carbs_g: 17
  fat_g: 0.7
  fiber_g: 2
  protein_g: 1.8
portions:
  default_g: 5
scientific_name: Zingiber officinale
sources:
  - USDA FoodData Central
  - NIH National Center for Complementary and Integrative Health
tags:
  - te
  - asiatico
  - antiinflamatorio
---

# Scientific Analysis & Culinary Use

## Description
El jengibre es un rizoma picante y aromático. Esencial en la cocina china y la medicina tradicional.

## Culinary Uses
- **Salteados:** Junto al ajo y cebollín.
- **Té:** Infusión para la gripa o náuseas.

## Health Benefits
### Antiemético Natural
Es el remedio no farmacológico más validado para las náuseas del embarazo y mareo por movimiento.
